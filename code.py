...
def duplicate_and_unique_movies(dataset, index_):
    ...
    duplicate=[]
    for i in dataset:
        if i[index_] in unique:
            duplicate.append(i[index_])
        else:
            unique.append(i[index_])
    print(len(duplicate))
    print(duplicate)
...
def rate_bucket(dataset, rate_low, rate_high):
    ...
    rated_movies=[]
    for i in dataset:
        if float(i[-4])<=rate_high and float(i[-4])>=rate_low:
...
for i in already_added:
    for j in range(len(movies_clean)):
        if movies_clean[j][-2]==i[-2] and movies_clean[j][-3]<i[-3]:
            movies_clean[j]=i
